...
// GetAddressByName 根据名字获取地址
func (service *AddressService) GetAddressByName(ctx context.Context, name string) serializer.Response {
	code := e.SUCCESS
	addressDao := dao.NewAddressDao(ctx)
	address, err := addressDao.ListAddressByName(name)
	if err != nil {
		logging.Info(err)
		code = e.ErrorDatabase
		return serializer.Response{
