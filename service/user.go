...
func (service UserService) Register(ctx context.Context) serializer.Response {
	...
	return serializer.Response{
		Status: code,
		Msg:    e.GetMsg(code),
		Data:   serializer.BuildUser(user).UserName,
	}
...
// Login 用户登陆函数
func (service *UserService) AdminLogin(ctx context.Context) serializer.Response {
	var user *model2.User
	code := e.SUCCESS
	userDao := dao2.NewUserDao(ctx)
	user, exist, err := userDao.ExistOrNotByUserName(service.UserName)
	...
	token, err := util.GenerateToken(user.ID, service.UserName, 1)
	if err != nil {
		logging.Info(err)
		code = e.ErrorAuthToken
		return serializer.Response{
...
// Update 用户查询其他用户信息
func (service UserService) Search(ctx context.Context, Username string) serializer.Response {
	var user *model2.User
	var err error
	// var exist bool
	code := e.SUCCESS
	// 找到用户
	userDao := dao2.NewUserDao(ctx)
	user, _, err = userDao.ExistOrNotByUserName(service.UserName)
	if err != nil {
		logging.Info(err)
		code = e.ErrorDatabase
		return serializer.Response{
...
func (service *SendEmailService) Send(ctx context.Context, id uint) serializer.Response {
	...
	m.SetHeader("Subject", "仅为vzt邮箱功能测试")
