...
func (service *ProductService) Create(ctx context.Context, uId uint, files []*multipart.FileHeader) serializer.Response {
	var boss *model2.Admin
	var err error
	code := e.SUCCESS

	AdminDao := dao2.NewAdminDao(ctx)
	boss, _ = AdminDao.GetAdminById(uId)
	// 以第一张作为封面图
	tmp, _ := files[0].Open()
	var path string
	...
	productDao := dao2.NewProductDao(ctx)
	_, exist, err := productDao.ExistOrNotByProductName(service.Name)
	if err != nil {
		code = e.ErrorDatabase
		return serializer.Response{
			Status: code,
...
// 自己乱写的
func (service *ProductService) SearchProduct(ctx context.Context, id string) serializer.Response {
	code := e.SUCCESS
	productDao := dao2.NewProductDao(ctx)
	pid, _ := strconv.Atoi(id)
	product, err := productDao.GetProductById(uint(pid))
