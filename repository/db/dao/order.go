...
func (dao *SkillGoodsDao) IncrementStock(productID uint, amount int) error {
	return dao.DB.Model(&model2.SkillGoods{}).
		Where("id = ?", productID).
		Update("num", gorm.Expr("num + ?", amount)).Error
}

// UpdateOrder 更新订单信息
func (dao *OrderDao) UpdateOrder(order *model2.Order) error {
