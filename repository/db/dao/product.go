...
func (dao *ProductDao) ExistOrNotByProductName(ProductName string) (product *model.Product, exist bool, err error) {
	var count int64
	err = dao.DB.Model(&model.Product{}).Where("name=?", ProductName).Count(&count).Error
	if count == 0 {
		return product, false, err
	}
	err = dao.DB.Model(&model.Product{}).Where("name=?", ProductName).First(&product).Error
	if err != nil {
		return product, false, err
	}
